...
void Chess::print_board() const {
    ...
    for (int row = BOARD_SIZE - 1; row >= 0; --row) {
        ...
        for (int col = 0; col < BOARD_SIZE; ++col) {
            // Tarkistetaan, onko ruutu musta vai valkoinen
            bool is_black_square = (row + col) % 2 == 0;

            // Sijoitetaan nappulat
            auto piece = board_.get_piece_at(row, col);
            if (piece != nullptr) {
                char symbol = '?';
                ...
                // Jos ruudulla ei ole nappulaa, tulostetaan ruutumerkki
                if (is_black_square) {
                    std::cout << "x ";  // Musta ruutu
                } else {
                    std::cout << ". ";  // Valkoinen ruutu
                }
            }
        }
        std::cout << row + 1 << "\n";
