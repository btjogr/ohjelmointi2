...
void TwoWayList::remove_duplicates()
{
    ...
    while (current) {
        shared_ptr<List_item> checker = current->next;

        while (checker) {
            if (current->data == checker->data) {
                shared_ptr<List_item> removable = checker;
                checker = checker->next;  // Siirrytään seuraavaan ennen rakenteen muuttamista

                // Päivitetään osoittimet, jotta "removable" poistetaan listasta
                if (removable->prev) {
                    removable->prev->next = removable->next;
                } else {
                    first_ = removable->next;  // Päivitetään first_, jos ensimmäinen alkio poistetaan
                }

                if (removable->next) {
                    removable->next->prev = removable->prev;
                } else {
                    last_ = removable->prev;  // Päivitetään last_, jos viimeinen alkio poistetaan
                }

                --count_;  // Päivitetään alkioiden määrä
            } else {
